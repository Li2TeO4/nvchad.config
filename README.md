...
lua/plugins/init.lua      插件清单（LSP/Mason/补全/Git/snacks/render-markdown…）
...
- **小窗口自动停用**：窗口可显示行数 ≤ 10（小分屏等场景）时自动停用自定义滚动，恢复原生行为，避免 4+4 行保护占满窗口；
- 如需调整余量，改 `scrollpad.lua` 顶部的 `PAD_BELOW` / `PAD_ABOVE`（小窗口阈值改 `MIN_WIN_HEIGHT`）。
...
| 主题 | `<leader>th` | 主题切换 |
| 帮助 | `<leader>?` / `<leader>ch` / `<leader>wK` | 配置 README（只读渲染）/ 键位速查表 / which-key 总览 |
